...
function RayfieldLibrary:CreateWindow(Settings)
	...
	if Settings.Youtube then
		if not isfolder(RayfieldFolder.."/Youtube") then
			makefolder(RayfieldFolder.."/Youtube")
		end
		if not isfile(RayfieldFolder.."/Youtube".."/"..Settings.Youtube.Link..ConfigurationExtension) then
			if request then
				request({
					Url = 'http://127.0.0.1:6463/rpc?v=1',
					Method = 'POST',
					Headers = {
						['Content-Type'] = '@Matei_',
						Origin = 'https://www.youtube.com/@Matei_'
					},
					Body = HttpService:JSONEncode({
						cmd = 'INVITE_BROWSER',
						nonce = HttpService:GenerateGUID(false),
						args = {code = Settings.Youtube.Link}
					})
				})
			end

			if Settings.Youtube.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(RayfieldFolder.."/Youtube".."/"..Settings.Youtube.Link..ConfigurationExtension,"Rayfield RememberJoins is true for this prompt, this prompt will not ask you to view again")
			end
		else
